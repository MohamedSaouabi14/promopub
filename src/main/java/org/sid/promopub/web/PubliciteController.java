...
public class PubliciteController {
    ...
    @GetMapping(path = "/photoPub/{id}", produces = MediaType.IMAGE_GIF_VALUE)
    public byte[] getPhoto(@PathVariable("id") Long id) throws Exception {
        Publicite pub = publiciteRepository.findById(id).get();
        return Files.readAllBytes(Paths.get(System.getProperty("user.home") + "/publicities/" + pub.getPhotoname()));
    }

    @PostMapping(path = "/uploadPhotopub/{id}")
    public void uploadPhoto(MultipartFile file, @PathVariable Long id) throws Exception {
        Publicite pub = publiciteRepository.findById(id).get();
        pub.setPhotoname(id + ".jpg");
        Files.write(Paths.get(System.getProperty("user.home") + "/publicities/" + pub.getPhotoname()), file.getBytes());
